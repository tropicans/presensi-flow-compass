...
import { Calendar, Download, Filter, Search, Users, UserCheck, Clock, BarChart3 } from 'lucide-react';
...
export const AttendanceDashboard: React.FC<AttendanceDashboardProps> = ({ records }) => {
  ...
  const filteredRecords = useMemo(() => {
    return records.filter(record => {
      ...
      const matchesDate = !dateFilter || 
                         format(record.waktu_presensi, 'yyyy-MM-dd') === dateFilter;
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      ...
      ...filteredRecords.map(record => [
        ...
        record.kegiatan,
        record.tipe_user,
        format(record.waktu_presensi, 'dd/MM/yyyy HH:mm')
      ].join(','))
    ].join('\n');
  ...
  const stats = useMemo(() => {
    ...
    const todayCount = records.filter(r => 
      format(r.waktu_presensi, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd')
    ).length;
  ...
  const activityStats = useMemo(() => {
    const stats = mockActivities.map(activity => ({
      name: activity.nama_kegiatan,
      count: records.filter(r => r.kegiatan === activity.nama_kegiatan).length
    }));
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Total Presensi</p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{stats.internalCount}</p>
              </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Eksternal</p>
                <p className="text-2xl font-bold">{stats.externalCount}</p>
              </div>
              ...
              <div key={index} className="p-4 border rounded-lg">
                ...
                <div className="w-full bg-muted rounded-full h-2 mt-2">
                  <div 
                    ...
                    style={{ width: `${stats.totalAttendance > 0 ? (stat.count / stats.totalAttendance) * 100 : 0}%` }}
                  />
                </div>
              </div>
            ))}
      ...
      <Card>
        ...
        <CardContent>
                ...
                {filteredRecords.length === 0 ? (
                  ...
                  filteredRecords.map((record) => (
                    <TableRow key={record.id}>
                      ...
                      <TableCell>{record.nip || '-'}</TableCell>
                      <TableCell>{record.instansi}</TableCell>
                      <TableCell>{record.kegiatan}</TableCell>
                      <TableCell>
                        <Badge variant={record.tipe_user === 'internal' ? 'default' : 'secondary'}>
                          {record.tipe_user === 'internal' ? 'Internal' : 'Eksternal'}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        {format(record.waktu_presensi, 'dd/MM/yyyy HH:mm')}
          ...
          <div className="mt-4 text-sm text-muted-foreground">
            Menampilkan {filteredRecords.length} dari {records.length} total presensi
          </div>
        </CardContent>
      </Card>
    </div>
